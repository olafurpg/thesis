...
Instead of reifying a Space into the literal \texttt{" "} for a particular \texttt{formatToken},
the FormatWriter builds up a \texttt{tokenAligns: Map[FormatToken, Int]} and reifies the split into the literal \texttt{" " * (1 + tokenAlign(formatToken))}.
Listing~\ref{alg:align} shows a rough sketch of how the \texttt{tokenAligns} map is constructed.
\begin{algorithm}
  \caption{Vertical alignment}\label{alg:align}
