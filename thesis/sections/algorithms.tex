...
Because the higher order function can have an arbitrary number of statements the priority queue grows out bounds.
...
An OptimalToken is a hint to the best-first search that indicates if one search branch can reach to a particular token at cost 0, the best-first search can eliminate competing branches.
By eliminating competing branches, we drastically minimize the search space.
How is this implemented?

Remember that a Split has an \texttt{optimalToken} member.
Listing~\ref{lst:optimalToken} shows the definition of OptimalToken.
\lstinputlisting[label={lst:optimalToken}, caption=OptimalToken definition]{code/optimalToken.scala}

A split can specify an \texttt{OptimalToken} at which if
