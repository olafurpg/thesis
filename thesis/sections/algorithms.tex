...
In a nutshell, the job of the \texttt{Router} is to convert each \texttt{FormatToken} into a \texttt{Split}

\subsubsection{Decision}\label{sec:decision}
A \texttt{Decision} is a pair of a \texttt{FormatToken} and a sequence of Splits.
...
% The \texttt{isProhibitive} member annotates whether this policy can possibly eliminate Splits from future \texttt{Decision}s.
...
Listing~\ref{lst:indent} shows the definition of Indent along with the algebraic data type \texttt{Length}.
...
The StateColumn placeholder is required to allow memoization of Splits, which is critical for performance reason as explained in section~\ref{sec:router} on the \texttt{Router}.
...
\subsubsection{State}
A \texttt{State} is a partial formatting solution during the best-first search.
...
The \texttt{compare} method orders States firstly by their \texttt{totalCost} member, secondly by \texttt{splits.length} -- how many \texttt{FormatToken}s have been formatted -- and finally breaking ties by the \texttt{indentation}.
...
The LineWrapper is responsible for turning \texttt{FormatToken}s into Splits.
...
\begin{figure}
  ...
  \label{fig:router}
\end{figure}
In this figure, the \texttt{Router} is the planner that has chosen to open up multiple branches at \texttt{=} and \texttt{+} and only one branch for the remaining tokens.
...
Listing~\ref{lst:match} shows how we can pattern match on a \texttt{FormatToken} and produce Splits.
\begin{minipage}{\linewidth}
...
In this small example, the bug may seem obvious but once the \texttt{Router} grows bigger the compiler becomes unmissable.
...
The ability to trace the origin of each Split has been indispensable in the development of the \texttt{Router}.
...
\end{algorithm}
The \texttt{isSafe} method on State returns true if the state contains prohibitive policies, derived from annotated metadata in Splits from the \texttt{Router}.
...
% The previous sections have explained the building blocks for how scalafmt formats a source file.
% In this section we take a simple example to show how all concepts work together.
% We will go through the steps of formatting the listing in the introduction.
%
