...
We run micro benchmarks to get insight into how scalafmt performs in an interactive software developer workflow.
For example, it is common to format a file on every save.
...
\end{table}
Observe that there is a gap in the ratio between file sizes.
The ratio is 11.51 for the Large file and 5.00 for the Small file, over 2x difference.
...
\begin{table}
  ...
  \caption{Percentiles of lines of code per file in macro benchmark. For example, 25th percentile of files have 16 lines of code.}~\label{tab:macro_sample}
\end{table}
...
\begin{table}
  \centering
  \input{target/macro.tex}\label{tab:macro}
