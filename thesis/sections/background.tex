...
\rfmt{} is a code formatter for the programming language and statistical programming environment \emph{R}.
The formatter was released in 2016 -- after the background work on this thesis started -- and like its optimization based forerunners is also developed at Google.
...
\rfmt{} uses 6 layout combinators, or \emph{blocks} as they are called in the report, to express all formatting rules for the R programming language.
...
\end{itemize}
As we will see, these relatively few combinators allow an impressive amount of flexibility.
...
Listing\ref{lst:rfmt-example} shows how we can express the choice between the two layouts in the combinator algebra above.
...
The blocks are responsible for expressing the formatting layouts.
We calculate \emph{layout functions} to choose an optimal layout.

To find an optimal layout, \rfmt{} employs a smart dynamic programming trick.
Firstly, a na\"{\i}ve approach would be to enumerate all possible combinations of layouts and choose the one 


To efficiently calculate an optimal layout for a given block.
\rfmt{} makes
The layout function is used to efficiently find an formatting layout.
The
Optimizations.
