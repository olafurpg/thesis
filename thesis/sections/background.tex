...
One such dialect is SBT configuration files, as discussed in section~\ref{sec:sbt}.
SBT adds custom support for top-level statements in \texttt{*.sbt}, which would otherwise result in a parse error using the Scala compiler parser.
...
After parsing, the tree

Tree node types in scala.meta preserve absolute fidelity of the original source file.
This means that we can obtain all syntactic details such as whitespace positions and comments from any tree node type.
This feature cannot be taken for granted as obvious as it may seem.

In scalafmt, this feature was instrumental 

Tokens in scala.meta are strongly typed.
Traditional object-oriented libraries treat tokens as a single type with multiple methods such as \texttt{isComma} which returns true if a token instance is a comma or not.
However, scala.meta leverages algebraic data types in Scala to represent each different kind of token as a separate type.
This enables perform powerful pattern matching on token types.
This feature was useful in the design of the \emph{Router} described in section~\ref{sec:router}.
...
Mark van der Brand presented a library that generates a formatter given a context-free grammar\autocite{van_den_brand_generation_1996}.
...
\lstinputlisting[numbers=none, label={lst:rfmt-example}, float,
